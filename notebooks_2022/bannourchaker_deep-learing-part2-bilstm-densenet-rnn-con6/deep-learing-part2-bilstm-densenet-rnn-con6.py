...
class Config:
    ...
    weight_decay = 2e-5
    epochs =175
    scheduler = "ReduceLROnPlateau"  # : ReduceLROnPlateau: CosineDecayRestarts#Custum
    early_stop = True
    seq_len = 60
    masking = False
...
# ## Many-to-One Model
# 
# ![image.png](attachment:219c0400-ec5a-4197-a320-2f7efc36a9b7.png)
# 
...
def LSTM_v1(x_pre):
    """Ref:https://www.kaggle.com/code/tenffe/finetune-of-tensorflow-bidirectional-lstm/notebook
    """
    with strategy.scope():
        n_features = x_pre.shape[-2:]
        model = tf.keras.models.Sequential([
            tf.keras.layers.Input(shape=train.shape[-2:]),
...
# # BiLSTM with ResNet-like structure :DenseNet
# **A Bidirectional LSTM, or biLSTM**, is a sequence processing model that consists of two LSTMs: one taking the input in a forward direction, and the other in a backwards direction. BiLSTMs effectively increase the amount of information available to the network, improving the context available to the algorithm (e.g. knowing what words immediately follow and precede a word in a sentence).
# 
# ![image.png](attachment:8fbbfcc4-f66e-49c6-9738-8585b1954cfa.png)
# 
# In our **DenseNet-BiLSTM**, the DenseNet is primarily applied to obtain local features, while the BiLSTM is used to grab time series features. In general,
# 
# Links:
# 
# - return sequence:
# 
# 1. https://stackoverflow.com/questions/42755820/how-to-use-return-sequences-option-and-timedistributed-layer-in-keras
# 
# - densenet :
# 
# 1. https://www.youtube.com/watch?v=ahkBkIGdnWQ
#  
# 1. https://www.analyticsvidhya.com/blog/2021/08/all-you-need-to-know-about-skip-connections/
# 
# 1. https://d2l.ai/chapter_convolutional-modern/densenet.html

# In[ ]:


def BiLSTMDenseNet(x_pre):   
    with strategy.scope():

        input_layer = tf.keras.layers.Input(shape=(x_pre.shape[-2:]))
        x1 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(768, return_sequences=True))(input_layer)

        x21 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(512, return_sequences=True))(x1)
        x22 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(512, return_sequences=True))(input_layer)
        l2 = tf.keras.layers.Concatenate(axis=2)([x21, x22])

        x31 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(384, return_sequences=True))(l2)
        x32 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(384, return_sequences=True))(x21)
        l3 = tf.keras.layers.Concatenate(axis=2)([x31, x32])

        x41 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(256, return_sequences=True))(l3)
        x42 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(128, return_sequences=True))(x32)
        l4 = tf.keras.layers.Concatenate(axis=2)([x41, x42])

        l5 = tf.keras.layers.Concatenate(axis=2)([x1, l2, l3, l4])
        x7 = tf.keras.layers.Dense(128, activation='selu', kernel_initializer='lecun_normal')(l5)
        x8 = tf.keras.layers.Dropout(0.3)(x7)
        output_layer = tf.keras.layers.Dense(units=1, activation="sigmoid")(x8)
        model = tf.keras.Model(inputs=input_layer, outputs=output_layer, name='DNN_Model')
...
simple_basic_dl =BiLSTMDenseNet(X)
#model_reg.summary()
