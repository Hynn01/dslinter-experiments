...
# 随机森林模型
model = RandomForestClassifier(n_estimators=100) # 实例化模型RandomForestClassifier
model.fit(x_train_scale, y_train)  # 在训练集上训练模型
#score = model.score(x_train_scaled,y_train)  #训练好的模型在训练集上进行评分，返回精确度
#print(score)
score = cross_val_score(model,x_train_scale,y_train,cv=4)
...
#利用建立的随机森林模型进行预测
clf = RandomForestClassifier(criterion='entropy',max_features=1,n_estimators=160)
