...
# 
# I aim to share the following information:
# 
# - CNN, although popular in image datasets, can also be used (and may be more practical than RNNs) on time series data
# 
# - Present a popular architecture for time series classification (univariate AND multivariate) called Fully Convolutional Neural Network (FCN)
# 
...
# 
# ![image.png](attachment:e06df082-ebbd-4ec5-ad6a-2528f54686e7.png)
# 
# I recommend to take a look at FCN first : 
# 
# https://www.kaggle.com/bannourchaker/deep-learing-part3-cnn-isallwhatyouneed-con8
# 
# https://www.kaggle.com/bannourchaker/deep-learing-part3-cnn-denstnet-con9
...
# # Now, why should we use CNN and not RNN? 
# 
# I will lay down some points:
# 
...
# In a way, CNNs can look forward: RNN models only learn from data before the timestep it needs to predict. CNNs (with shuffling) can see data from a broader perspective.
...
# 
# As always, in AI and ML, you can only make sure by consulting to domain experts, referencing to research papers or other learning resources, and by doing actual experiments.
# # Inception 
# **Motivation:** Increasing the depth (number of layers) is not the only way to make a model bigger.
# 
# **What about increasing both the depth and width of the network while keeping computations to a constant level?**
# 
# ![image.png](attachment:4fcd090d-0e68-432f-913b-ee74b8a12c34.png)
# 
...
# 
# ![image.png](attachment:923fa6a2-4216-444f-b7ee-35df1c1dc330.png)
# 
...
# 
# * The output of the bottleneck is fed to three one-dimensional convolutional layers of kernel size .
#  
...
def _inception_module( input_tensor, nb_filters,use_bottleneck,bottleneck_size,kernel_size,stride=1, activation='linear'):
...
def Inception(x_pre, nb_classes=60,nb_filters=32,bottleneck_size = 32,
    ...
    with strategy.scope():
        ...
        for d in range(depth):
            ...
            x = _inception_module(x,nb_filters=32,use_bottleneck=True,bottleneck_size = 32, kernel_size=41)
        ...
        out = tf.keras.layers.Dense(nb_classes, activation='sigmoid')(gap_layer)

        
        _model = tf.keras.Model(inputs=input_layer, outputs=out)
