...
get_ipython().run_cell_magic('time', '', '## try circle crop\nNUM_SAMP=7\nfig = plt.figure(figsize=(25, 16))\nfor class_id in sorted(train_y.unique()):\n    for i, (idx, row) in enumerate(df_train.loc[df_train[\'diagnosis\'] == class_id].sample(NUM_SAMP, random_state=SEED).iterrows()):\n        ax = fig.add_subplot(5, NUM_SAMP, class_id * NUM_SAMP + i + 1, xticks=[], yticks=[])\n        path=f"../input/aptos2019-blindness-detection/train_images/{row[\'id_code\']}.png"\n        image = circle_crop(path,sigmaX=30)\n\n        plt.imshow(image)\n        ax.set_title(\'%d-%d-%s\' % (class_id, idx, row[\'id_code\']) )')
...
get_ipython().system('ls ../input/diabetic-retinopathy-resized/')


# In[ ]:


get_ipython().system('ls ../input/diabetic-retinopathy-resized/resized_train/resized_train | head')
...
get_ipython().system('ls ../input/retinopathy-train-2015/rescaled_train_896/rescaled_train_896/ | head')
