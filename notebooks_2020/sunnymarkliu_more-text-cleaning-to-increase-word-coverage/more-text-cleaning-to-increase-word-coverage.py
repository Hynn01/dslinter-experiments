...
get_ipython().run_cell_magic('time', '', 'oov_glove, oov_paragram, oov_fasttext, oov_google = vocab_check_coverage(train_df, test_df)')
...
get_ipython().run_cell_magic('time', '', 'train_df = df_parallelize_run(train_df, text_clean_wrapper)\ntest_df = df_parallelize_run(test_df, text_clean_wrapper)')
...
for vocab in cur_vocabulary:
    if '-' in vocab:
        ...
        if (vocab in embed_glove or vocab.capitalize() in embed_glove or vocab.lower() in embed_glove) and             (vocab in embed_paragram or vocab.lower() in embed_paragram):
...
get_ipython().run_cell_magic('time', '', 'train_df["question_text"] = train_df["question_text"].apply(spacing_dash_point)\ntest_df["question_text"] = test_df["question_text"].apply(spacing_dash_point)')
...
get_ipython().run_cell_magic('time', '', 'train_df = df_parallelize_run(train_df, fix_dash_point_spacing_bug_wrapper)\ntest_df = df_parallelize_run(test_df, fix_dash_point_spacing_bug_wrapper)')
